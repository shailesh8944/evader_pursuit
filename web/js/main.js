...
// Works with vessel_state format:
...
class VesselManager {
  ...
  discoverVessels() {
    ...
    if (typeof ros.getTopicsAndTypes === 'function') {
      ros.getTopicsAndTypes((result) => {
        ...
        const vesselTopics = topics.filter(t => t.endsWith('/vessel_state') || t.endsWith('/odometry_sim'));
  ...
  createVisualizersFromList(topicList) {
    // topicList can contain either '/name/vessel_state' or '/name/odometry_sim'
    const names = new Set();
    topicList.forEach(t => {
      try {
        const parts = t.split('/');
...
class VesselVisualizer {
  ...
  subscribe() {
    ...
    // Subscribe to vessel_state; the message is std_msgs/Float64MultiArray (per your info)
    const stateTopicName = `/${this.vesselName}/vessel_state`;
    try {
