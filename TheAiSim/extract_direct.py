...
def get_message_type(topic_type):
    ...
    if topic_type == 'nav_msgs/msg/Odometry':
        ...
        return Float64
    elif topic_type == 'interfaces/msg/Actuator' and ACTUATOR_MSG_AVAILABLE:
        return Actuator
    else:
...
def process_bag_directory(bag_dir, output_dir):
    """
    Process a ROS2 bag directory and extract data to CSV files
    ...
    for topic, msg_type in topic_type_map.items():
        if 'imu/data' in topic:
            imu_topic = topic
        elif 'odometry' in topic and 'sim' in topic:
            odom_topic = topic
    ...
    print(f"  Actuator: {actuator_topic}")
    ...
    while reader.has_next():
        ...
        # Process Rudder encoder data
            ...
            try:
                ...
                print(f"Error processing rudder message: {e}")
        
        # Process Actuator command data
        elif topic_name == actuator_topic:
            try:
                if ACTUATOR_MSG_AVAILABLE:
                    msg = deserialize_message(data, Actuator)
                else:
                    # Generic approach if Actuator message type not available
                    msg_type = get_message_type(topic_type_map[topic_name])
                    if not msg_type:
                        print(f"Skipping actuator message - no message type")
                        continue
                    msg = deserialize_message(data, msg_type)
                ...
                print(f"Error processing actuator message: {e}")
    ...
    if all(topic in dataframes for topic in ['imu', 'odom', 'rudder']):
...
def prepare_training_data(dataframes, time_steps=1):
    """
    Prepare training data for the neural network model
    ...
    """
    if 'synchronized' not in dataframes:
        raise ValueError("Synchronized data not available")
    ...
    for i in range(len(df) - time_steps):
        # Input features
        features = df.iloc[i][['pos_x', 'pos_y', 'vel_x', 'vel_y', 'accel_x', 'accel_y', 'ang_vel_z', 'rudder_angle', 'yaw']].values
...
def main():
    parser = argparse.ArgumentParser(description='Extract data from ROS2 bags using rclpy')
    ...
    print(f"Found {len(bag_dirs)} run directories: {bag_dirs}")
    ...
    for bag_dir in bag_dirs:
        try:
            print(f"\nProcessing bag directory: {bag_dir}")
    ...
    if successful_csv_files:
        ...
        print(f"\nExtracted data from {len(successful_csv_files)} bag directories")
