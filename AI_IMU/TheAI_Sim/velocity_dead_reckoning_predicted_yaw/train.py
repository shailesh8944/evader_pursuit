...
def train_model(data_dir, num_epochs=50, batch_size=32, learning_rate=0.001, model_save_path='state_delta_predictor_fnn.pth', val_split_ratio=0.1):
    """
    ...
    if val_split_ratio > 0 and val_split_ratio < 1.0:
        ...
        if train_size <= 0 or val_size <=0:
            print(f"Train or validation size is 0. Training on full dataset. Train: {train_size}, Val: {val_size}")
    ...
    # 4. Training Loop
    print("Starting training for delta predictor...")
    ...
    for epoch in range(num_epochs):
        ...
        running_loss = 0.0
        for i, (inputs, delta_targets) in enumerate(train_loader):
            inputs, delta_targets = inputs.to(device), delta_targets.to(device)

            # Forward pass
            predicted_deltas = model(inputs)
            loss = criterion(predicted_deltas, delta_targets)
        ...
        if val_loader:
            ...
            val_loss = 0.0
            with torch.no_grad():
                for inputs_val, delta_targets_val in val_loader:
                    inputs_val, delta_targets_val = inputs_val.to(device), delta_targets_val.to(device)
                    predicted_deltas_val = model(inputs_val)
                    loss_val = criterion(predicted_deltas_val, delta_targets_val)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train State Delta Predictor FNN Model')
